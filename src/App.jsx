...
function LandingPage({ onStart }) {
  const container = { maxWidth:1024, margin:"0 auto", padding:"0 20px", width:"100%" }
  const card = { background:L["container-lowest"], borderRadius:10, boxShadow:"0px 4px 16px rgba(0,0,0,0.04)", border:`1px solid ${L["outline-variant"]}` }

  return (
    <div style={{ background:L.surface, minHeight:"100vh", fontFamily:"Inter, -apple-system, sans-serif" }}>
      {/* Top Navigation */}
      <header style={{ position:"fixed", top:0, left:0, right:0, zIndex:50, background:L["container-lowest"], borderBottom:`1px solid ${L["outline-variant"]}` }}>
        <div style={{ ...container, display:"flex", justifyContent:"space-between", alignItems:"center", height:56 }}>
      ...
      <main style={{ paddingTop:56 }}>
        {/* Hero compacta */}
        <section style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:"calc(100vh - 56px)", textAlign:"center", padding:"60px 20px" }}>
          <div style={{ maxWidth:680 }}>
            <h1 style={{ fontSize:40, fontWeight:700, color:L.primary, lineHeight:1.15, margin:"0 0 12px", letterSpacing:"-0.02em" }}>
              Análisis de Mercado Inmobiliario con <span style={{ color:L.secondary }}>IA</span>
            </h1>
            <p style={{ fontSize:16, color:L["on-surface-variant"], lineHeight:1.5, maxWidth:480, margin:"0 auto 32px" }}>
              Precios, tendencias y comparables al instante para propiedades en Colombia.
            </p>
            {/* Search bar funcional */}
            <div style={{ display:"flex", flexWrap:"wrap", gap:6, justifyContent:"center", margin:"0 auto" }}>
              <div style={{ flex:"1 1 200px", display:"flex", alignItems:"center", gap:6, padding:"8px 14px", background:L["container-lowest"], borderRadius:8, border:`1px solid ${L["outline-variant"]}` }}>
                <span>📍</span>
                <input placeholder="Ciudad" style={{ border:"none", outline:"none", fontSize:14, width:"100%", background:"transparent" }}/>
              </div>
              <div style={{ flex:"1 1 160px", display:"flex", alignItems:"center", gap:6, padding:"8px 14px", background:L["container-lowest"], borderRadius:8, border:`1px solid ${L["outline-variant"]}` }}>
                <span>🏘️</span>
                <input placeholder="Barrio" style={{ border:"none", outline:"none", fontSize:14, width:"100%", background:"transparent" }}/>
              </div>
              <div style={{ flex:"1 1 140px", display:"flex", alignItems:"center", gap:6, padding:"8px 14px", background:L["container-lowest"], borderRadius:8, border:`1px solid ${L["outline-variant"]}` }}>
                <span>🏷️</span>
              ...
              <button onClick={onStart} style={{ background:L.primary, color:L["on-primary"], border:"none", borderRadius:8, padding:"8px 20px", fontSize:14, fontWeight:600, cursor:"pointer", whiteSpace:"nowrap" }}>
                Analizar
              </button>
            </div>
            <p style={{ fontSize:12, color:L["on-surface-variant"], marginTop:16 }}>
              🔍 Scraping en vivo + IA · Medellín, Bogotá, Cali, Barranquilla y más
            </p>
          </div>
        </section>
        ...
        <section style={{ background:L["container-low"], padding:"48px 0" }}>
          <div style={{ ...container, textAlign:"center" }}>
            <h2 style={{ fontSize:24, fontWeight:600, margin:"0 0 32px" }}>¿Cómo funciona?</h2>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:16 }}>
              {[
                {icon:"🕸️", title:"Scraping", desc:"Extraemos datos de Metrocuadrado, FincaRaíz y otras fuentes en tiempo real."},
                {icon:"🧠", title:"IA Analiza", desc:"Claude AI procesa los datos y calcula precios de mercado, oportunidad y aspiración."},
                {icon:"📄", title:"Reporte", desc:"Recibes un informe completo con gráficos, mapas y recomendaciones."},
              ].map((item,i) => (
                <div key={i} style={{ ...card, padding:20 }}>
                  <div style={{ fontSize:28, marginBottom:10 }}>{item.icon}</div>
                  <h4 style={{ fontSize:16, fontWeight:600, margin:"0 0 6px" }}>{item.title}</h4>
        ...
        {/* Ciudades */}
        <section style={{ ...container, padding:"48px 0", textAlign:"center" }}>
          <h2 style={{ fontSize:24, fontWeight:600, margin:"0 0 24px" }}>Cobertura en Colombia</h2>
          <div style={{ display:"flex", flexWrap:"wrap", gap:8, justifyContent:"center" }}>
            {["Medellín","Bogotá","Cali","Barranquilla","Cartagena","Bucaramanga","Pereira","Manizales","Santa Marta","Cúcuta","Ibagué","Villavicencio"].map(c => (
              <span key={c} style={{ background:L["container-lowest"], borderRadius:20, padding:"6px 16px", fontSize:13, fontWeight:500, border:`1px solid ${L["outline-variant"]}` }}>{c}</span>
            ))}
          </div>
        </section>
        ...
        <section style={{ ...container, paddingBottom:48 }}>
          <div style={{ background:L.primary, borderRadius:12, padding:"32px 24px", textAlign:"center" }}>
            ...
            <p style={{ fontSize:14, color:"rgba(255,255,255,0.7)", margin:"0 0 20px" }}>Obtén el valor real de mercado con datos actualizados.</p>
      ...
      <footer style={{ borderTop:`1px solid ${L["outline-variant"]}`, padding:"16px 0" }}>
