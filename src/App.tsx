...
type PhaseProps = { f: AnalysisForm; s: React.Dispatch<React.SetStateAction<AnalysisForm>> }
type AppView = "landing" | "auth" | "onboarding" | "dashboard" | "analysis"
...
// ── ATOMS ─────────────────────────────────────────────────────────────────────
const Card = ({ children, style = {}, glow = false }: { children: React.ReactNode; style?: React.CSSProperties; glow?: boolean }) => (
  <div style={{
...
const Sel = ({ value, onChange, options, placeholder }: { value: string; onChange: (v: string) => void; options: Array<SelectOption | string>; placeholder?: string }) => (
...
const SearchSelect = ({ value, onChange, options, placeholder = "Busca barrio...", disabled }: {
  ...
  const filtered = (normalizedQuery ? list.filter(b => b.toLowerCase().includes(normalizedQuery)) : list).slice(0, 25)
  const handleSelect = (val: string) => { onChange(val); setQuery(val); setOpen(false) }
  return (
    <div style={{ position: "relative" }}>
      <input
        ...
        style={{
          ...
          cursor: disabled ? "not-allowed" : "text", transition: "border .2s"
        }}
      />
      {open && !disabled && filtered.length > 0 && (
        <div style={{
          ...
          {filtered.map(b => (
            <div key={b} onMouseDown={e => e.preventDefault()} onClick={() => handleSelect(b)}
              style={{ padding: "8px 12px", fontSize: 13.5, cursor: "pointer", color: b === value ? C.green : C.white, background: b === value ? C.bg2 : "transparent" }}>
              {b}
            </div>
          ))}
          {filtered.length === 25 && <div style={{ padding: "6px 12px", fontSize: 11, color: C.gray }}>Sigue escribiendo para afinar la búsqueda…</div>}
        </div>
      )}
    </div>
  )
...
const PhaseTitle = ({ icon, title, sub }: { icon: string; title: string; sub?: string }) => (
...
// ── LOCATION MAP ──────────────────────────────────────────────────────────────
function LocationMap({ form }: { form: AnalysisForm }) {
...
// ── MAP VIEW ──────────────────────────────────────────────────────────────────
function MapView({ form, result }: { form: AnalysisForm; result: AnalysisResult }) {
  const mapRef = useRef<HTMLDivElement>(null)
...
const Results = ({ form, result, onReset, user, onDashboard }: ResultsProps) => {
...
export default function App() {
  ...
  const [form, setForm] = useState<AnalysisForm>({ ...EF })
...
// Retained for potential future use with barrio autocomplete
export { SearchSelect, CIUDADES_POR_PAIS }
