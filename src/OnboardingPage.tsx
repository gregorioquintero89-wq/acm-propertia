...
export default function OnboardingPage({ user, onComplete }: OnboardingPageProps) {
  ...
  const [companyName, setCompanyName] = useState<string>(user?.user_metadata?.company_name ?? "")
  const [isSaving, setIsSaving] = useState(false)
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [logoPreview, setLogoPreview] = useState<string | null>(null)
  const [logoUrl, setLogoUrl] = useState<string>(user?.user_metadata?.logo_url ?? "")
  ...
  const handleLogoSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      if (typeof ev.target?.result === "string") setLogoPreview(ev.target.result)
    }
